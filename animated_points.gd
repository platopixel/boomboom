...
func _ready():
	...
	var target_modulate = Color(self.modulate.r * modifier, self.modulate.g, self.modulate.b, 0.3)
	...
	tween.parallel().tween_property(self, "modulate", target_modulate, modified_timer)
